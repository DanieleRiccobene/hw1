function onDelete(json){
    ...
    content.innerHTML = '';
    fetch("queryFlight.php").then(onResponse).then(onJson);
}
...
function deleteFlight(event){
    ...
    fetch("deleteFlight.php",options).then(onResponse).then(onDelete);
}
...
function onJson(json){
    ...
    fetch("checkDateFlight.php").then(onResponse).then(onShowDate);
    numEl = json.length;
