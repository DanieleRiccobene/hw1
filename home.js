function addBaggage(){
    window.location.href = "addBaggage.php";
}

function onConfirmJson(json){
    console.log(json);
    if(!json.state){
        error = json.error;
        alert(error);
...
function prenotaVolo(event) {
    ...
    fetch("addFlight.php",options).then(onConfirm).then(onConfirmJson);
...
function getIataPart(city) {
    ...
    fetch("obtainIata.php", options).then(onResponseIata).then(onIataJsonPart);
}
...
function getIataDest(city) {
    ...
    fetch("obtainIata.php", options).then(onResponseIata).then(onIataJsonDest);
}
...
function search(event) {
    ...
    fetch("obtainFlights.php", options).then(onResponse).then(onFlightJson);
}
...
fetch("obtainToken.php").then(onTokenJson);
